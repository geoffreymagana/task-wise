...
export default function DashboardPage() {
  ...
  const defaultTaskCount = useMemo(() => {
    return tasks.filter(task => task.status === 'not_started' && isSameDay(new Date(task.createdAt), currentDate)).length;
  }, [tasks, currentDate]);
  ...
  const renderTaskCount = () => {
    ...
    if (activeTab === 'active') {
       const todayTasks = activeTasks.filter(t => t.dueDate && isSameDay(new Date(t.dueDate), currentDate)).length;
